...
enum Token {
    NUM, IDENT, OP_ARIT,
    ABRE_PAR, FECHA_PAR, ABRE_COL, FECHA_COL, ABRE_CHA, FECHA_CHA,
    T_EOF, VAZIO
};
...
AnalisadorLexico(const std::string& nomeArquivo) {
        ...
        std::string linha;
        entrada = "";
        while (std::getline(arquivo, linha)) { //le o arquivo linha por linha
            entrada += linha + "\n"; 
        }
        arquivo.close();
    ...
    void s0() {
        ...
        else if (proxCaractere == '(') { leProxCaractere(); s7_apar(); }
        else if (proxCaractere == ')') { leProxCaractere(); s8_fpar(); }
        else if (proxCaractere == '[') { leProxCaractere(); s9_acol(); }
        else if (proxCaractere == ']') { leProxCaractere(); s10_fcol(); }
        else if (proxCaractere == '{') { leProxCaractere(); s11_acha(); }
        else if (proxCaractere == '}') { leProxCaractere(); s12_fcha(); }
        else {
            throw ErroLexico(proxCaractere);
        }
    }

    // Estado s1: Reconhecimento de Números Inteiros
    void s1() {
    ...
    // Estado s2: Reconhecimento de Identificadores (Variáveis)
    void s2() {
        token_Reconhecido = IDENT;
    ...
    // Estados dos Operadores e Símbolos (Equivalentes aos estados simples do Java)
    ...
    // Estado de Fim de Arquivo
    void s_eof() { token_Reconhecido = T_EOF; }
};

// 3. PROGRAMA PRINCIPAL DE TESTE (Baseado no TesteAnalisadorLexico.java do professor)
int main(int argc, char* argv[]) {
    ...
    try {
     AnalisadorLexico scanner(argv[1]);
        ...
        // Executa a máquina de Moore repetidamente até encontrar o Fim de Arquivo (EOF)
        do {
            scanner.s0();
            ...
            switch (scanner.token_Reconhecido) {
                ...
                case IDENT:     std::cout << "<IDENT> "; break;
                case OP_ARIT:   std::cout << "<OP_ARIT> "; break;
                case ABRE_PAR:  std::cout << "<ABRE_PAR> "; break;
                case FECHA_PAR: std::cout << "<FECHA_PAR> "; break;
                case ABRE_COL:  std::cout << "<ABRE_COL> "; break;
                case FECHA_COL: std::cout << "<FECHA_COL> "; break;
                case ABRE_CHA:  std::cout << "<ABRE_CHA> "; break;
                case FECHA_CHA: std::cout << "<FECHA_CHA> "; break;
                case T_EOF:     std::cout << "<EOF>\n"; break;
                default: break;
            }
